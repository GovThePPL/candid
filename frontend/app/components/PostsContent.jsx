...
export default function PostsContent({ onScroll, listHeader, stickyHeader, userId: targetUserId }) {
    ...
    try {
      ...
      if (!cursor) {
        const cacheKey = isOtherUser
          ? `activity:posts:other:${targetUserId}`
        ...
        const result = await fetchApi({ type: 'posts', cursor })
