...
// Desktop card scaling: cards render at this reference width and scale down to fit narrower panels
...
export default function ModerationQueueContent() {
  const { t } = useTranslation('moderation')
  const colors = useThemeColors()
  const isDesktop = useIsDesktop()
  const styles = useMemo(() => createStyles(colors), [colors])

  // Desktop card scaling: render cards at CARD_REFERENCE_WIDTH and scale to fit panel
  ...
  const cardScale = isDesktop && containerWidth > 0
    ? Math.min(1, containerWidth / CARD_REFERENCE_WIDTH)
    : 1
  ...
  if (loading) {
    return <ModerationQueueSkeleton styles={styles} colors={colors} />
  }
  ...
  return (
    <View style={styles.container} onLayout={isDesktop ? handleContainerLayout : undefined}>
      <View
        style={[
          styles.cardScrollWrapper,
          isDesktop && {
            width: CARD_REFERENCE_WIDTH,
            alignSelf: 'center',
            transform: [{ scale: cardScale }],
            transformOrigin: 'top center',
            marginBottom: wrapperHeight > 0 ? -(wrapperHeight * (1 - cardScale)) : 0,
          },
        ]}
        onLayout={isDesktop ? handleWrapperLayout : undefined}
      >
        <ScrollView style={styles.cardContainer} contentContainerStyle={styles.cardContainerContent}>
