...
| `sessions_controller.py` | Sessions | Sessions (formerly categories), NLP-suggested sessions |
...
Admin (location-scoped, inherits DOWN the location tree)
  └─ Moderator (location-scoped, inherits DOWN)
      └─ Facilitator (location + session scoped, NO inheritance)
          ├─ Assistant Moderator (location + session scoped)
          ├─ Expert (location + session scoped)
          └─ Liaison (location + session scoped)
```
...
- **Admin and Moderator** are location-only (no session). The DB enforces `session_id IS NULL`.
- **Facilitator and below** are location + session scoped. A facilitator at "Oregon" for "Education" is separate from "Oregon" for "Healthcare".
...
| `authorization_scoped(role, token_info, location_id, session_id)` | Primary role check. Walks hierarchy + location ancestors. If no `location_id`, checks any role anywhere. |
| `authorization_site_admin(token_info)` | Root admin only. For global operations (e.g. creating sessions). |
...
| `is_facilitator_for(user_id, location_id, session_id)` | Facilitator at exact location+session. |
| `get_highest_role_at_location(user_id, location_id, session_id)` | User's most powerful role at a location. |
| `has_qa_authority(user_id, location_id, session_id)` | Check if user has a qualifying role for Q&A answers. |
...
| Facilitator | `assistant_moderator`, `expert`, `liaison` | At their exact location + session |
...
- *Facilitator-assignable roles:* peer facilitator at same location+session → location moderator → location admin → auto-approve
...
**`user_role`** — Active role assignments. Admin/moderator require `location_id`, forbid `session_id`. Facilitator and below require `location_id`, session optional. Unique indexes prevent duplicates (separate indexes for with/without session due to NULL handling).

**`role_change_request`** — Audit trail. Tracks action, target user, role, location, session, requester, authority location, reason, status, reviewer, denial reason, auto-approve deadline, and timestamps.

**`location`** — Self-referential tree (`parent_location_id`). Soft-deletable (`deleted_at`). Deleting reparents children to parent.

**`location_session`** — Maps which sessions are available at which locations.
...
The frontend receives `user.roles[]` (array of `{role, locationId, sessionId}`) from `GET /users/me`.
...
| `proposal_context.py` | Context gathering for AI-guided proposal creation (wiki pages, glossary, Q&A, expert content scoped to a session) |
| `serializers.py` | Shared serialization helpers: canonical user card builder, creator sub-object, pagination clamping |
| `wiki.py` | Wiki/glossary helper functions: suggestion processing, three-way merging, glossary upsert, image cleanup |
