"""Tests for stats endpoints: GET /stats/{locationId}/{sessionId},
GET /stats/{locationId}, GET /stats/{locationId}/{sessionId}/demographics/{groupId}."""
...
class TestGetStats:
    """GET /stats/{locationId}/{sessionId}"""

    def test_get_stats_success(self, normal_headers):
        """Can get stats for a location and session."""
        resp = requests.get(
            f"{STATS_URL}/{OREGON_LOCATION_ID}/{HEALTHCARE_SESSION_ID}",
    ...
    def test_invalid_location_404(self, normal_headers):
        ...
        resp = requests.get(
            f"{STATS_URL}/{NONEXISTENT_UUID}/{HEALTHCARE_SESSION_ID}",
            headers=normal_headers,
        )
        assert resp.status_code == 404

    def test_invalid_session_404(self, normal_headers):
...
class TestGetLocationStats:
    ...
    def test_get_location_stats_success(self, normal_headers):
        """Can get stats for a location (all sessions)."""
        resp = requests.get(
...
class TestGetGroupDemographics:
    """GET /stats/{locationId}/{sessionId}/demographics/{groupId}"""

    def test_get_demographics_all_groups(self, normal_headers):
        """Can get demographics for 'all' group."""
        resp = requests.get(
            f"{STATS_URL}/{OREGON_LOCATION_ID}/{HEALTHCARE_SESSION_ID}/demographics/all",
            headers=normal_headers,
        )
        assert resp.status_code == 200
        body = resp.json()
